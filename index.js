...
const { menuseven } = require('./src/menuseven')
...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=+558294174972:+55 (82) 9417-4972\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerB: '[❗] Este comando só pode ser usado pelo Ban! ❌',
					admin: '[❗] Este comando só pode ser usado por administradores de grupo! ❌',
					Badmin: '[❗] Este comando só pode ser usado quando o bot é admiro! ❌',
                                        daftarB: `── 「REGISTRE-SE」 ──\nSLV mano como!\nVocê nn está registrado no banco de dados criador tem que se cadastrar pra ter acesso ao menu, \n\nComando : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar fulano|19`,
				}
			}
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '5582994174972@s.whatsapp.net'
                        /******ApiKey Input******/
				...
				case 'demitir':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `Não mamou o adm direito,se fudeu :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`Puts,qm não sabe mamar os adm direito vira membro comum😂 @${mentioned[0].split('@')[0]}\n*${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
                  ...
                  case 'promover':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Esse mamou geral pra virar admiro :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Esse deve ter dado uma mamada nos adm : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
  ...
  client.updatePresence(from, Presence.composing) 
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'criador':
                case 'creator':
                  client.sendMessage(from, {displayname: "ban", vcard: vcard}, MessageType.contact, { quoted: mek})
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'wikien':
					...
					break				
				case 'ytmp3':
					if (args.length < 1) return reply('Onde está o link?eu vou ter q tirar do toba ou oq?!?')
	...
	case 'kbbi':
               ...
               if (data.error) return reply(data.error)
				...
				case 'adminlist':
					client.updatePresence(from, Presence.composing) 
                                        if (!isUser) return reply(mess.only.daftarB)
					if (!isGroup) return reply(mess.only.group)
					teks = `Lista dos cara mais Pika da *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'reportar':
					client.updatePresence(from, Presence.composing) 
                                        if (!isUser) return reply(mess.only.daftarB)
					if (!isGroup) return reply(mess.only.group)
					teks = `Marcando Tds os adms,para q um deles conserte o bug *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
					for (let admon of groupAdmins) {
						no += 1
						teks += `[${no.toString()}] @${admon.split('@')[0]}\n`
				...
				case 'setprefix':
					...
					if (!isGroupAdmins) return reply(mess.only.ownerB)
					prefix = args[0]
				...
				case 'teste':
					meme = await kagApi.memes()
					buffer = await getBuffer(`http://imgur.com/gallery/1YJRCOu.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '.......'})
					break
				case 'teste1':
					meme = await kagApi.memes()
				...
				case 'teste2':
					meme = await kagApi.memes()
					buffer = await getBuffer(`http://imgur.com/gallery/1YJRCOu${meme.hash}.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '.......'})
					break
				case 'teste3':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`http://imgur.com/gallery/1YJRCOu.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nome: Ban \nnúmero:wa.me/+558294174972 \nCara gostoso do krlh,chama ae'})
					break
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSBAVlGRYWLQRtvMrs9kPkrX8UfMYRt7XPpkg&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nome: Ban \nnúmero:wa.me/+558294174972 \nCara gostoso do krlh,chama ae'})
					break
                case 'menu':
		client.sendMessage(from, menu(prefix, sender), text, {quoted: mek})
                break
                case 'menuseven':
		client.sendMessage(from, menuseven(prefix, sender), text, {quoted: mek})
                break
                case 'midia':
		client.sendMessage(from, midia(prefix, sender), text, {quoted: mek})
                break
                case 'download':
		client.sendMessage(from, download(prefix, sender), text, {quoted: mek})
                break
                case 'menuvip':
		client.sendMessage(from, menuvip(prefix, sender), text, {quoted: mek})
                break
                case 'tag':
	         	if (args.length < 1) return reply ('࿇  ͥ ͣ ͫ♕nome ༒࿇')
		        break
                case 'testedoif':
	         	if (funcione) return reply('parabens ta funcionando')
	         	if (args.length < 1) return client.sendMessage(from, 'Qual é o código da linguagem?', text, {quoted: mek})
	         	if (args.length < 1) return reply ('࿇  ͥ ͣ ͫ♕nome ༒࿇')
		        break
                case 'nome':
				...
				case 'bv':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRe3u719Fi9hpEg1oOGjZ_sNWHyFvEzdXKWtg&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Oiie,bem vindo\nMeu nome é Zero Two\nSe precisar de mim digita\n!Menu'})
					break
				case 'canal':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTlP1G16OauLLUCuLk5c-QmngF78PMEtNJOYQ&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Se inscreva e deixe o like pra dar uma moral \nCanal: https://www.youtube.com/channel/UCZ8_Fyb9YmQt8UBeWclTuuQ/videos'})
					break
				case 'hentai':
		     		if (!isGroupAdmins) return reply(mess.only.admin)
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS3pVAbO_tiF9kHkwKTFXPS1cfStbm-YG8tMQ&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'hehe😏\nAdemir Querendo Ver hentai\nuiii kkk'})
					break
				case 'hentai2':
		     		if (!isGroupAdmins) return reply(mess.only.admin)
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQtbJs7FGvrpwr1AV0srNfhph8oooWBViH-Ew&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Só tem 3 entt aproveitem'})
					break
				case 'hentai3':
		     		if (!isGroupAdmins) return reply(mess.only.admin)
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSkdFbGeVHJ2DlMMD3J3qyk2ZCisDFu-98RWQ&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'hehe\npunheteiros é fogo'})
					break
				case 'adm':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTZn07ASgyBc5eZZOYkbdGnYMaCLcqyZHtPYQ&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Queremos o adm Ban\naiii o adm mais gostoso da facç 👉👈'})
					break
				case 'fbi':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTv6_ske5nMIhODEzw70cH6bVkR1D28GHFQpA&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Hmmm,entt vc quer lolis\npodem levar garotos'})
					break
				case 'fbi1':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSx0l50HsqAnmoXByIHwo7MUx5Me2eOdJgvDg&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Roi amigo,Buscando lolis n é msm?'})
					break
				case 'memeindo':
					memein = await kagApi.memeindo()
				...
				case 'block':
					...
					client.sendMessage(from, `Entendi chefe,Vou bloquear esse fela da mãe ${body.slice(8)}@c.us`, text)
					break
				case 'hilih':
					client.updatePresence(from, Presence.composing) 
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 ZeroTwo 〙', text, {quoted: mek})
					break
                case 'tagall3':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.The)
					members_id = []
					teks = (args.length > 1) ? body.slice(8).trim() : ''
					teks += `  Total : ${groupMembers.length}\n`
					for (let mem of groupMembers) {
					...
					client.sendMessage(from, '╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 ZeroTwo 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				if (!isQuotedImage) return reply(`Envie fotos com legendas ${prefix}setbotpp ou tags de imagem que já foram enviadas`)
					if (!isGroupAdmins) return reply(mess.only.ownerB)
					enmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contexno
			...
			case 'igstalk':
               ...
               client.updatePresence(from, Presence.composing) 
              options = {
          text: `Este é o Dono da Poha Toda : @${from.split("-")[0]}`,
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'adicionar':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Você quer add qm?')
				...
				case 'remover':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('A marca o otario q vc quer remover!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Flw Otario :\n'
						for (let _ of mentioned) {
						...
						mentions(`Blz Chefia,vou retirar esse pau no cu : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
				...
				case 'txtdafoto':
                    ...
                    reply('Problemas foram relatados ao proprietário da zero-two, relatórios falsos não serão respondidos.')
                    break
               case 'apakah':
               client.updatePresence(from, Presence.composing) 
	    ...
	    case 'kapankah':
            ...
            break
				...
				case 'ttp':
					...
					break
                                case 'clearall':
					if (!isGroupAdmins) return reply('Kamu siapa?')
					anu = await client.chats.all()
