	      ...
	      tels = body.slice(6)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'botinfo':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTOW1nL6im90TIO68-WGhhY672ga5q4dObQ0Q&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nome:Zero Two \nNumero:http://Wa.me/+558896904592 \nData de criação:30/11/2020'})
					break
				case 'canal':
					memein = await kagApi.memeindo()
