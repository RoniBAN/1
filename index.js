			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				case 'hentai':
					...
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTKyCaHeK4GEpxMQ2TwDWKKv1LFRE9nEHNslA&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'queremos o adm satã\n😳\n👉👈'})
					break
				case 'hentai2':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSfcitgm3cYY48xHdTPNDdc5F-MXbGLEdq76qpeSMQSyDFkQGs-M92Mz6_a&s=10`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'so tem mais essa satã'})
					break
				case 'dono':
					memein = await kagApi.memeindo()
				...
				case 'teste':
                    client.updatePresence(from, Presence.composing) 
					data = await fetchJson(`https://www.sohentaidobom.com/2015/03/imagens-aleatorias-01-jpg.html?m=1&zx=d362ce874ab8325a`, {method: 'get'})
                                        if (!isUser) return reply(mess.only.daftarB)
					reply(mess.wait)
					n = JSON.parse(JSON.stringify(data));
					nimek =  n[Math.floor(Math.random() * n.length)];
